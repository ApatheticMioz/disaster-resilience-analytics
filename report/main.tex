...
\begin{itemize}
    ...
    \item \textbf{Asia}: Mean CRI = 8.1, diverse range from Japan to Yemen
    \item \textbf{Africa}: Mean CRI = 4.9, lowest regional average
\end{itemize}

\subsection{Temporal Evolution (Time)}
The CRI calculation incorporates INFORM data (available from 2016), which affects the absolute scale. Comparing within consistent methodology periods: pre-2016 mean CRI improved from 7.23 (2000) to 8.18 (2015), representing 13.1\% growth. Table~\ref{tab:temporal} shows regional trajectory divergence in the readiness-vulnerability gap.
...
\midrule
Earthquake & 20,290 & High intensity, localized \\
Wildfire & 3,051 & Seasonal, infrastructure damage \\
Flood & 2,126 & Most frequent, widespread \\
Tropical Cyclone & 405 & Severe, coastal regions \\
Drought & 240 & Slow-onset, prolonged recovery \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key Finding}: While earthquakes dominate event counts in GDACS (20,290 alerts), the integrated DII and RRS calculations aggregate impacts at the country-year level. Drought-affected countries typically exhibit lower median RRS values, as slow-onset disasters strain recovery capacity more severely despite generating fewer alert-level events.
...
Japan, Chile, and New Zealand fall in the ``Fighters'' quadrant---high DII, high RRS. Japan experienced 150+ disasters yet maintains CRI = 14.9 through institutional resilience (Fig.~\\ref{fig:quadrant}). New Zealand achieves CRI = 29.3 despite high seismic exposure.
...
\begin{enumerate}
    ...
    \item \textbf{Causality}: Correlational analysis cannot establish causal direction. While governance correlates with resilience ($r = 0.71$), reverse causality---resilient institutions enabling good governance---cannot be ruled out without instrumental variable approaches.
...
Row Count & 4,608 (192 countries $\times$ 24 years) \\
Column Count & 102 \\
Year Range & 2000--2023 \\
Geographic Scope & 192 UN member states \\
